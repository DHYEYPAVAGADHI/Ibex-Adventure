  ...
  return (
    <>
      ...
      <main className="bg-[var(--color-sand)] min-h-screen">
        ...
        {/* Filters Bar */}
        <div className="bg-white border-b border-[var(--color-hair)] sticky top-[72px] z-30 shadow-sm">
          <div className="container-shell max-w-7xl mx-auto flex flex-wrap items-center justify-between gap-4 py-4 px-6 md:px-0">
            <div className="flex flex-wrap gap-4 w-full md:w-auto">
              {['DESTINATION', 'EXPERIENCE', 'DURATION', 'MONTH'].map((filter, i) => (
                <button key={i} className="flex-1 md:flex-none flex items-center justify-between gap-2 px-4 py-3 bg-[var(--color-sand)] hover:bg-[#eee] rounded border border-[var(--color-hair)] text-xs font-bold uppercase tracking-widest text-[var(--color-ink)] transition-colors">
                  {filter}
                  <ChevronDown className="w-4 h-4 text-[var(--color-ink-muted)]" />
                </button>
              ))}
            </div>
            <div className="text-xs font-bold uppercase tracking-widest text-[var(--color-ink-muted)] w-full md:w-auto text-right">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {journeys.map((journey) => (
                <Link
                  ...
                  className="group flex flex-col bg-white border border-[var(--color-hair)] rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-shadow duration-300"
                >
                  ...
                  <div className="p-6 flex flex-col flex-grow">
                    <h3 className="font-sans text-2xl font-black uppercase text-[var(--color-ink)] mb-4">
                      {journey.title}
                    </h3>
                    
                    <div className="flex flex-wrap gap-4 text-xs font-bold uppercase tracking-widest text-[var(--color-ink-muted)] mb-6">
                    ...
                    <div className="mt-auto pt-6 border-t border-[var(--color-hair)] flex items-center justify-between">
                      <div className="flex flex-col">
                        <span className="text-[10px] font-bold uppercase tracking-widest text-[var(--color-ink-muted)]">FROM</span>
