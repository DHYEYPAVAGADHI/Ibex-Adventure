...
export function AttractionDeleteButton({ id }: { id: string }) {
  ...
  if (confirming) {
    return (
      <div className="flex items-center gap-2">
        ...
        <button onClick={() => setConfirming(false)} className="text-xs text-admin-muted hover:text-admin-heading">No</button>
      </div>
    );
  }
